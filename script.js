...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateChart() {
        ...
        // Only add other categories if they have a value
        if (expense > 0) {
            labels.push('Expense');
            dataArr.push(expense);
            bgColors.push('#f44336');
        }
        if (savings > 0) {
            labels.push('Savings');
            dataArr.push(savings);
            bgColors.push('#ff9800');
        }
        if (balance > 0 && (expense > 0 || savings > 0)) {
            labels.push('Balance');
            dataArr.push(balance);
    ...
    // On page load, render any saved transactions and chart
    renderTransactions();
    updateSummary();
    updateChart();
